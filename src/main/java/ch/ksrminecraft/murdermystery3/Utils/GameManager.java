...
public class GameManager {
    ...
    public GameManager(PointsManager pointsManager, MurderMystery3 plugin) {
    ...
    public void handleJoin(Player p) {
        ...
        if (players.add(uuid)) {
            ...
            FileConfiguration config = MurderMystery3.getInstance().getConfig();
    ...
    public void startGame() {
        ...
        broadcastToPlayers(ChatColor.YELLOW + "Spiel startet");

        // RoleManager.assignRoles erwartet jetzt Set<UUID>
        ...
        for (Map.Entry<UUID, Role> e : roles.entrySet()) {
            ...
            if (p != null && p.isOnline()) {
                ...
                // Items je nach Rolle bei Rundenstart verteilen
                switch (e.getValue()) {
                    case DETECTIVE -> {
                        p.getInventory().clear();
    ...
    public void resetGame() {
        ...
        if (mainWorld != null) {
            ...
            Bukkit.getLogger().severe("Hauptwelt '" + mainWorldName + "' wurde nicht gefunden!");
        }
    ...
    private void checkWinConditions() {
        ...
        if (!murdererAlive) {
            ...
            if (MurderMystery3.getInstance().isMurdererKilledByBow()) {
